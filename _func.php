...
function renderNav(PageArray $items) {
    ...
    foreach($items as $item) {
        ...
        $out .= "<a href='$item->url'>$item->title</a> ";
           ...
           // if the item has images, include that too
        $out .= "<ul class='image'><li><img src='{$item->images->first()->url}'></li></ul>";

        // $out .= "<ul class='bg'><li></li></ul>";
    ...
    if($out) $out = "<div class='col-xs-7'></div><div class='col-xs-5 project-links'><div class='project-links-fixed'><ul class='nav'>$out</ul></div></div>\n";
...
//Display images if available
    if(count($page->images)) {
        ...
        echo "</div>";
        echo "</div>";
        echo "<div class='col-xs-5'><h1>{$page->title}</h1><p>{$page->body}</p></div>";
    }
